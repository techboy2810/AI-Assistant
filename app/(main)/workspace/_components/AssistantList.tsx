...
function AssistantList() {
  ...
  const GetUserAiAssistants = async () => {
    const result = await convex.query(
      api.userAiAssistants.GetAllUserAiAssistants,
      {
        uid: user._id,
      }
    );
  ...
  `}
            ...
            <div>
              ...
              <h2 className="text-gray-600 text-sm dark:text-gray-50">
                {assistant_.title}
              </h2>
            </div>
          </div>
      ...
      <div className="absolute bottom-10 flex gap-3 items-center hover:bg-gray-200 w-[87%] p-2 rounded-xl cursor-pointer">
        {user?.picture && (
          <Image className="rounded-full" src={user.picture} alt="user" width={35} height={35} />
        )}
        <div>
